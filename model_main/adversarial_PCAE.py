"""
This file is run second to train the adversarial Variational Autoencoder
"""
...
class AAE():
    ...
    def train(self, epochs, batch_size=128, sample_interval=50, n_critic=5,da_states=1): 
        ...
        if da_states == 1:
            ...
            def da_state_images():
                pickle_path = 'C:\\Users\\Philip\\states_pickled'
                file = open(pickle_path, 'rb')
                da_state = pickle.load(file)

                file.close()
                return da_state

            #optional experiments: use this to augment latent state with other states of experimental images
            def da_state_augment(da_state,modelPcs,pca=1):

                if pca == 1:
                    #apply PCA on states
                    da_state = np.array(da_state)
                    da_shaped = da_state.reshape(np.shape(da_state)[0],np.shape(da_state)[1]*np.shape(da_state)[2])
                    pca = PCA(30) #nr of PCs
                    da_state_pca = pca.fit_transform(da_shaped) 

                    pca_extended = np.concatenate([da_state_pca,modelPcs],axis=1)


                #merge full state vectors, very high dimensional, not recomended
                else: 
                    pca_extended = []
                    for i in range(np.shape(da_state)[0]):
                        da_state[i].reshape(np.shape(da_state)[1]*np.shape(da_state)[2],1)
                        pca_extended.append(np.concatenate([modelPcs[i,:], da_state[i].reshape(np.shape(da_state)[1]*np.shape(da_state)[2])] ,axis=None))
                return np.array(pca_extended)

            states = da_state_images()
            ls_scaled = da_state_augment(states,ls_scaled) 
...
if __name__ == '__main__':
    ...
    #define state augmentation experiments, 0 for default without augmentation
    da_states = 0
    if da_states == 1:
        #how many dimensions are added to state space
        npcs = 70 
