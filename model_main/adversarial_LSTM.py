"""
This file is run third and trains the LSTM component for forecasting in latent space. 
Note that you can reproduce the physical space just without the prediction component and just using the Adv. VAE.
"""
...
class ALSTM():
    ...
    def train(self, epochs, batch_size=128, sample_interval=50, n_critic=5):
        ...
        global pcs_trun, ls_scaled
        pcs_trun = np.load(self.directory_data + '/' + 'pcs_' + self.field_name + '_' + 'data_'+str(start_interv)+'_to_'+str(end_interv)+ '.npy')
        # Load encoder
        generator_enc = tf.models.load_model(self.directory_data + '/' + 'AAE_MV_generator_encoder_Full_'+self.GANorWGAN
                                            +'_' + self.field_name + '_' + str(latent_dim) + '_' + str(epochs))
    ...
    def plot_loss(self, epoch):
        ...
        plt.plot([i[2] for i in self.g_hist], c='orange')
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.title("GAN Loss per Epoch")
        plt.legend(['C real', 'C fake', 'Generator'])
        plt.subplot(1,2,2)
        plt.plot([i[1] for i in self.g_hist], c='green')
        plt.xlabel('Epoch')
        plt.ylabel('Mean squared error (-)')
        plt.title('MSE loss')
        plt.legend(['Training', 'Validation'])
        plt.tight_layout()
        plt.savefig(self.directory_data + '/' + 'Losses_AAE-PCAE_LSTM_' + GANorWGAN + '_noise_' +
    ...
    def plot_values(self, epoch):
        ...
        for i in range(latent_dim):#np.arange(8):
            plt.subplot(2, 4, i+1)
            plt.plot(y_all[:, :, i])
